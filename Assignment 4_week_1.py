# -*- coding: utf-8 -*-
"""
Created on Sat Jan 26 19:11:59 2019
...
def get_list_of_university_towns():
    uvTown_df = pd.read_fwf('university_towns.txt')
    ...
    with open('university_towns.txt') as file:
        for line in file:
            if '[edit]' in line:
                ...
                univ_town.append((state, line))
                
    uvTown_df = pd.DataFrame(univ_town, columns = ['State', 'RegionName'])
    
    def getStateCity(row):
        row['State'] = row['State'][:row['State'].find('[')]
        row['RegionName'] = row['RegionName'][:row['RegionName'].find('(')]
        return row

    uvTown_df.apply(getStateCity, axis=1)
    uvTown_df['RegionName'] = uvTown_df['RegionName'].str.strip()
    uvTown_df.iloc[96]['RegionName'] = "Champaign Urbana"
    return uvTown_df

def get_recession_start():
    cols = ['Quarter', 'GDP current', 'GDP chained']
    gdp_df = pd.read_excel('gdplev.xls', skiprows=5, parse_cols = "E:G").dropna()
    gdp_df.columns = cols
    gdp_df['Year'] = gdp_df['Quarter']
    
    def get_year(row):
        row['Year'] = (int)(row['Year'][:row['Year'].find('q')])
        return row
    
    gdp_df = gdp_df.apply(get_year, axis = 1)
    gdp_df = gdp_df.where(gdp_df['Year'] > 1999).dropna()
    gdp_df['diff'] = gdp_df['GDP chained'].diff()
    gdp_df = gdp_df.where((gdp_df['diff'] < 0) & (gdp_df['diff'].shift(-1) < 0)).dropna()
    return gdp_df.iloc[0]['Quarter']
...
def get_recession_end():
    ...
    gdp_df = gdp_df.apply(get_year, axis = 1)
    gdp_df = gdp_df.where(gdp_df['Year'] > 2008).dropna()
    gdp_df['diff'] = gdp_df['GDP chained'].diff()
    gdp_df = gdp_df.where((gdp_df['diff'] > 0) & (gdp_df['diff'].shift() > 0)).dropna()
    return gdp_df.iloc[0]['Quarter']
...
def get_recession_bottom():
    ...
    gdp_df = gdp_df.apply(get_year, axis = 1)
    gdp_df = gdp_df.where((gdp_df['Year'] > 2008) & (gdp_df['Year'] < 2010)).dropna()
    return gdp_df.loc[gdp_df['GDP chained'].idxmin()]['Quarter']
...
def convert_housing_data_to_quarters():
    ...
    housing_df = pd.read_csv('City_Zhvi_AllHomes.csv', usecols=[1,2,*range(51,251)],index_col=[1,0])
...
def run_ttest():
    ...
    recession_bottom = recession_bottom.replace('q', 'Q')
    housing_df = convert_housing_data_to_quarters()
    housing_df.columns = housing_df.columns.strftime('%YQ%q')
    housing_df["Growth"] = housing_df[recession_start]/housing_df[recession_bottom]
    
    uv_Town = get_list_of_university_towns()
    uv_Town.set_index(['State','RegionName'], inplace = True)
    #uv_Town.join(housing_df, on = ['State','RegionName'], how = 'left')
    #merge_df = pd.merge(uv_Town, housing_df, how='left', on = ['State','RegionName']).dropna()
    uv_df = housing_df.loc[housing_df.index.isin(uv_Town.index)]
    non_uv_df = housing_df.loc[~housing_df.index.isin(uv_Town.index)]
    
    x = ttest_ind(uv_df['Growth'], non_uv_df['Growth'], nan_policy = 'omit')
    touple = (True, x.pvalue, "university town")
    return touple
...
def q1test():
    ...
    print('Column test: ',"Passed" if df.columns.tolist() == cols else 'Failed')
    print('\\n test: ', "Failed" if any(df[cols[0]].str.contains('\n')) or any(df[cols[1]].str.contains('\n')) else 'Passed')
    print('Trailing whitespace test:',"Failed" if any(df[cols[0]].str.contains('\s+$')) or any(df[cols[1]].str.contains('\s+$')) else 'Passed')
    print('"(" test:', "Failed" if any(df[cols[0]].str.contains('\(')) or any(df[cols[1]].str.contains('\(')) else 'Passed')
    print('"[" test:',
